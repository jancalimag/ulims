	...
	*/	
	class PrintRequestView extends CGridView
	{
		...
		public function renderRow($row)
		{
			...
			self::$activeSheet->getStyle('A39:K46')->applyFromArray($outline);
			...
			self::$activeSheet->getStyle('J47:K47')->applyFromArray($bottom);
			self::$activeSheet->getStyle('A49:F50')->applyFromArray($outline);
			self::$activeSheet->getStyle('G49:K50')->applyFromArray($outline);
			
			self::$activeSheet->getStyle('A52:K53')->applyFromArray($outline);
			
			self::$activeSheet->getStyle('A55:K58')->applyFromArray($outline);
			self::$activeSheet->getStyle('A55:K55')->applyFromArray($outline);
			...
			self::$activeSheet->getStyle('A4')->getFont()->setSize(8);
			...
			self::$activeSheet->setCellValue('H11', 'Email:');

			self::$activeSheet->SetCellValue('A12', 'CUSTOsdfsfMER:');
			...
			self::$activeSheet->mergeCells('J9:K9');
			...
			self::$activeSheet->getStyle('D16:H36')->getAlignment()->setWrapText(true);
			
			self::$activeSheet->getStyle('C16:C36')->getAlignment()->setWrapText(true);
			...
			self::$activeSheet->SetCellValue('H47', 'TOTAL');
			self::$activeSheet->getStyle('I47')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			self::$activeSheet->SetCellValue('I47', 'P');
			...
			self::$activeSheet->getStyle('K47')->getNumberFormat()->setFormatCode('#,##0.00');
			self::$activeSheet->getStyle('I47')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
			...
			self::$activeSheet->getStyle('K8')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
			self::$activeSheet->getStyle('K8')->getFont()->setSize(8);
			self::$activeSheet->SetCellValue('K8', $this->formNum);
			
			self::$activeSheet->getStyle('J9')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
			self::$activeSheet->getStyle('J9')->getFont()->setSize(8);
			self::$activeSheet->SetCellValue('J9', $this->formRevNum. " | ". $this->formRevDate);
			...
			self::$activeSheet->SetCellValue('K47', $this->request->total);
	
			self::$activeSheet->SetCellValue('A49', 'OR NO.:');
			self::$activeSheet->SetCellValue('A50', 'DATE:');
			
			self::$activeSheet->SetCellValue('G49', 'AMOUNT RECEIVED:');
			self::$activeSheet->SetCellValue('G50', 'UNPAID BALANCE:');
			
			self::$activeSheet->SetCellValue('A52', 'REPORT DUE ON:');
			self::$activeSheet->SetCellValue('C52', date("M. j, Y", strtotime($this->request->reportDue)).' (4:00 PM)');
			
			self::$activeSheet->SetCellValue('A53', 'MODE OF RELEASE:');
			self::$activeSheet->SetCellValue('C53', 'Pick-up[  ]    Courier[  ]    Email[  ]    Fax[  ] ');
			self::$activeSheet->SetCellValue('C53', 'Pick-up[  ]    Courier[  ]    Email[  ]    Fax[  ] ');
			
			self::$activeSheet->SetCellValue('A55', 'DISCUSSED WITH CUSTOMER');
			self::$activeSheet->SetCellValue('A56', 'None payment of this Technical Service Request shall be considered cancelled on the due date given. "NO PAY NO TEST" policy.');
			self::$activeSheet->SetCellValue('A57', 'Return sample after testing.  YES[  ]   NO[  ] ');
			self::$activeSheet->SetCellValue('A58', 'I have agreed to the details including the Terms and Conditions stated in this Technical Service Request');
			
			self::$activeSheet->getStyle('A56')->getFont()->setSize(8);
			self::$activeSheet->getStyle('A57')->getFont()->setSize(8);
			self::$activeSheet->getStyle('A58')->getFont()->setSize(8);
			
			self::$activeSheet->SetCellValue('A59', 'CONFORME: ');
			
			self::$activeSheet->SetCellValue('D59', 'Sample/s Received by:');
			self::$activeSheet->SetCellValue('H59', 'Sample/s Reviewed by:');
		...
		public function run()
		{
			...
			if($this->logoLeft){
				...
				self::$objPHPExcelDrawing->setCoordinates('B3');
			...
			if($this->logoRight){
				...
				self::$objPHPExcelDrawing->setCoordinates('I3');
				self::$objPHPExcelDrawing->setHeight(60);
			}
